...
Logistic Regression
"""
# DesicionTrees.decision_trees(df, target_decision_tree, features_decision_tree)
# Regression.logisticRegression(df, 'InScotland')
...
# scaler, X_scaled = Clustering.elbow_method(df_copy, features_clustering)
# Clustering.k_means_clustering(df_copy, features_clustering, 3, scaler, X_scaled)
...
df_copy[target_regression] = df_copy[target_regression].map({'Slight': 0, 'Serious':1, 'Fatal': 2})

Regression.linearRegression(df_copy, features_regression, target_regression)

Regression.randomForestRegressor(df_copy, features_regression, target_regression)